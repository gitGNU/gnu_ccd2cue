...
error_push_f (const char function_name[], const char *template, ...)
{
  ...
  /* Allocate space on the error stack to the new error message. */
  error_stack = xrealloc (error_stack, sizeof (*error_stack) * ++error_entries);

  /* Store error message, with the supplied function name, on error
     stack's top */
  if (vasprintf (&str, template, ap) < 0)
    error (EX_OSERR, errno, "%s: could not push error to stack", __func__);
