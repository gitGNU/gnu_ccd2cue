 ...
 * that file on GNU+Linux-Libre system, but I only found a lot of people
 * asking for a solution on a lot of forums, and getting the unanimous
 * answer: no way!  In the first I could not believe that at this point
 * there was no option.  Then, with a little of patience and research, I
 * wrote a code to convert that files to a format much more common and
 * accessible, an ad-hoc standard in GNU operating system: the CUE Sheet
 * format.  So I could burn a lot of what I wanted!  I wondered whether
 * it would be useful for others...  And here is the result!
 *
 */
